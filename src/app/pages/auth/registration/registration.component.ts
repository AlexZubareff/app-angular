...
register(ev: Event):void | boolean {
  ...
  if(this.password !== this.passwordRepeat) {
    this.messageService.add({severity:'error', summary:'Service Message', detail:'Confirm password not equal password'});
    return false;
  }
  ...
  if(!this.authService.isUserExists(newUser)){
    ...
    this.messageService.add({severity:'success', summary:'Service Message', detail:'User added'});
    console.log('saveLocalStorageValue: ', this.saveLocalStorageValue);
    
  } else {
    this.messageService.add({severity:'warn', summary:'Service Message', detail:'User exists'});
  }
